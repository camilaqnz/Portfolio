...
export default function About() {
  return (
    <section id="about" className="bg-[#f8f8fc] px-16 py-32">
      <div className="flex items-end justify-between mb-20 pb-8 border-b border-[#e2e2ee]">
        <div>
          <p className="text-[11px] font-medium uppercase tracking-[0.12em] text-[#5563c8] mb-3">Sobre mí</p>
          <h2 style={{ fontFamily: 'var(--font-display)' }} className="text-[clamp(2rem,3.5vw,3rem)] text-[#121a52] tracking-tight leading-none">
      ...
      <div className="grid grid-cols-2 gap-28 items-start">
        <div>
          <h3 style={{ fontFamily: 'var(--font-display)' }} className="text-[1.875rem] text-[#121a52] leading-snug tracking-tight mb-8">
            Hola, soy Camila. Diseño experiencias digitales desde hace +4 años.
          </h3>
          <p className="text-[#5a5a7a] text-[15px] leading-[1.85] mb-7">
          ...
          <p className="text-[#5a5a7a] text-[15px] leading-[1.85] mb-10">
        ...
        <div className="bg-white border border-[#e2e2ee] rounded-2xl overflow-hidden">
          <div className="px-8 py-6 border-b border-[#e2e2ee] text-[11px] font-medium uppercase tracking-[0.08em] text-[#9898bb]">
            Herramientas &amp; habilidades
          </div>
          {SKILLS.map(({ name, pct }, i) => (
            <div key={name} className={`px-8 py-5 flex justify-between items-center ${i < SKILLS.length - 1 ? 'border-b border-[#f0f0f8]' : ''}`}>
              <span className="text-[15px] text-[#1e1e30]">{name}</span>
              <div className="w-28 h-1 bg-[#f0f0f8] rounded-full overflow-hidden">
                <div className="h-full rounded-full bg-[#7680d8]" style={{ width: `${pct}%` }} />
              </div>
            </div>
          ))}
        </div>
